...
- **Page**: registration SMS uses `/form?caseId=<UUID>&token=<HMAC>&mode=reg` and shows only one registration-number input. Callback SMS uses `/form?caseId=<UUID>&token=<HMAC>&mode=phone` (src/pages/FormSubmit.tsx). Do not send direct `functions/v1/iizi-reg-form` links to callers; that host returns HTML as `text/plain` on mobile.
...
- **Base URL**: orchestrator builds `form1_link` / legacy `form_link` as `LOCATION_PAGE_BASE_URL` + `/form?mode=reg` for registration-only collection. `form2_link` remains the callback-phone page using `LOCATION_PAGE_BASE_URL` + `/form?mode=phone`. Legacy `GOOGLE_FORM_BASE_URL` is a fallback only.
