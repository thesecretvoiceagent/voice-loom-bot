...
- **Page**: registration SMS uses the dedicated `iizi-reg-form` backend function with `caseId=<UUID>&token=<HMAC>` and shows only one registration-number input. Callback SMS still uses `/form?caseId=<UUID>&token=<HMAC>&mode=phone` (src/pages/FormSubmit.tsx).
...
- **Base URL**: orchestrator builds `form1_link` / legacy `form_link` as the dedicated `iizi-reg-form` URL for registration-only collection. `form2_link` remains the callback-phone page using `LOCATION_PAGE_BASE_URL` + `/form?mode=phone`. Legacy `GOOGLE_FORM_BASE_URL` is a fallback only.
