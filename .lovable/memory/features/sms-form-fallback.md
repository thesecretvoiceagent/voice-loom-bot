...
- **Page**: registration SMS uses `functions/v1/iizi-reg-form?caseId=<UUID>&token=<HMAC>` and shows only one registration-number input. Callback SMS uses `/form?caseId=<UUID>&token=<HMAC>&mode=phone` (src/pages/FormSubmit.tsx).
...
- **Base URL**: orchestrator builds `form1_link` / legacy `form_link` as the `iizi-reg-form` backend function for registration-only collection. `form2_link` remains the callback-phone page using `LOCATION_PAGE_BASE_URL` + `/form?mode=phone`. Legacy `GOOGLE_FORM_BASE_URL` is a fallback only.
