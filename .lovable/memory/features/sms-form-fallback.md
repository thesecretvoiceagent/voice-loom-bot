...
- **Base URL**: orchestrator builds `form1_link` for registration (`mode=reg`) and `form2_link` for callback phone (`mode=phone`) using `LOCATION_PAGE_BASE_URL` + `/form` path. Legacy `form_link` still resolves to the same registration-only URL for backward compatibility. Legacy `GOOGLE_FORM_BASE_URL` is a fallback only.
