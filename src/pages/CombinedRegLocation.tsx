...
type SubmitState =
  | { kind: "idle" }
  | { kind: "loading" }
  | { kind: "success"; reg: string; address: string }
...
export default function CombinedRegLocation() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const formRes = await supabase.functions.invoke("form-submit", {
        body: {
          caseId,
          token,
          reg_no: reg,
      ...
      if (formRes.error) {
        setSubmit({ kind: "error", message: formRes.error.message || "Registreerimisnumbri salvestus ebaõnnestus" });
        return;
      }
      if (!formRes.data?.ok) {
        setSubmit({ kind: "error", message: formRes.data?.error || "Registreerimisnumbri salvestus ebaõnnestus" });
        return;
      }

      const locationRes = await supabase.functions.invoke("location-confirm", {
        body: {
          caseId,
          token,
          lat: position[0],
      ...
      if (locationRes.error) {
        setSubmit({ kind: "error", message: locationRes.error.message || "Asukoha kinnitamine ebaõnnestus" });
        return;
      }
      if (!locationRes.data?.ok) {
        setSubmit({ kind: "error", message: locationRes.data?.error || "Asukoha kinnitamine ebaõnnestus" });
        return;
      }

      setSubmit({
        kind: "success",
        reg: formRes.data?.reg_no || reg,
    ...
    <div className="bg-background flex flex-col" style={{ minHeight: "100dvh" }}>
      <header className="px-5 pt-6 pb-3 space-y-2 shrink-0">
        <h1 className="text-2xl font-semibold text-foreground leading-tight">
          Registreerimisnumber ja asukoht
        </h1>
      ...
      <form
        ...
        className="flex-1 px-5 pb-[max(1rem,env(safe-area-inset-bottom))] space-y-3"
      >
        <div className="space-y-1.5">
          ...
          <input
            ...
            maxLength={12}
            disabled={submit.kind === "loading" || submit.kind === "success"}
        ...
        <div className="relative w-full bg-muted shrink-0 rounded-md overflow-hidden" style={{ height: "45vh", minHeight: 260 }}>
          {position ? (
        ...
        {token === "preview" && (
          <div className="rounded-md border border-warning/40 bg-warning/10 px-3 py-2 text-sm text-warning">
            Eelvaate link näitab lehte, aga ei salvesta. Päris test vajab allkirjastatud linki.
          </div>
        )}
        ...
        {submit.kind === "success" ? (
          ...
          >
            <div className="font-semibold">Andmed kinnitatud</div>
            <div className="text-sm opacity-90">Reg: <span className="font-mono">{submit.reg}</span></div>
            {submit.address && <div className="text-sm opacity-90">{submit.address}</div>}
          </div>
        ) : (
          <button
            type="submit"
            disabled={!position || !regNo.trim() || submit.kind === "loading"}
          ...
          >
            {submit.kind === "loading" ? "Kinnitan…" : "Kinnita"}
          </button>
        )}
      </form>
    </div>
